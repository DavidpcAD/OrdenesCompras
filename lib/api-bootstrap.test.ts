...
//   4. el cuerpo vuelve TAL CUAL vino además de parseado: ese texto es el que se
//      guarda en la caché del navegador para pintar de una la próxima vez.
...
test("la primera carga no manda If-None-Match y devuelve los datos", async () => {
  ...
  assert.deepEqual(b.datos, DATOS);
  // El texto crudo y el ETag viajan juntos: con eso se guarda la caché sin volver a
  // serializar 1,2 MB, y sin ETag la caché no podría pedir un 304 la próxima vez.
  assert.deepEqual(JSON.parse(b.texto), DATOS);
...
test("cuando cambia el ETag, entran los datos nuevos y se guarda el ETag nuevo", async () => {
  ...
  await api.bootstrap();
  assert.deepEqual((await api.bootstrap()).datos, DATOS2);
  await api.bootstrap();

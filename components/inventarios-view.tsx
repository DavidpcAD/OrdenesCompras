...
// Catálogo de artículos + existencias por almacén desde BC (inventoryByLocation).
...
export function InventariosView({ tablaKey = "inventarios" }: { tablaKey?: string }) {
  ...
  return (
    <main className="page page--wide">
      ...
      <div className="mt-4">
        <DataTable data={rows} columns={columns} tablaKey={tablaKey} getRowId={(a) => a.code} onRowClick={(a) => setSel(a)} rowClassName={(a) => (sel?.code === a.code ? "row-borrador" : "")} vacio="Sin artículos en el catálogo." />
      </div>
      ...
      {sel && (
        <div className="mt-6">
          ...
          <div className="mt-2">
            ...
            {estado === "ok" && (
              <DataTable data={existencias} columns={exColumns} tablaKey={`${tablaKey}-existencias`} vacio="Sin existencias." />
            )}
          </div>
        </div>
      )}
